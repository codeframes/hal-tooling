...
class ExplicitLinksProcessorTest extends Specification {
    ...
    def "test addLinks with same rel"() {
        setup:
          ...
          def linkProcessor = Spy(ExplicitLinksProcessor) {
              get({ it == bean }) >> links
          }
    ...
    def "test addLinks with different rel"() {
        setup:
          def linkMap = [:]
          def bean = new Object()
          def links = [new Link('rel_1', 'href_1'), new Link('rel_2', 'href_2')]
          def linkProcessor = Spy(ExplicitLinksProcessor) {
              get({ it == bean }) >> links
          }
